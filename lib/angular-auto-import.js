...
export default {
              ...
              ngModuleEditor.scan(insertInSection, (ngModulePosition) => {
                ...
                if (ngModuleEditor.buffer.getText().search(alreadyImported) === -1) {
                  ngModuleEditor.scan(/import/, (ngModuleImports) => {
                    ...
                    ngModuleEditor.insertText('import { ' + className + ' } from \'' + previousRelativePath.replace(/\\/g,"/") + '\';\n');
                    resolve(true);
                  });
                } else {
                  resolve(true);
  ...
  importdecorators() {
    ...
    if (editor) {
      ...
      for (decorator of firstDecorators) {
        let decoratorSearcher = RegExp('@' + decorator + '[^a-zA-Z]');
      ...
      for (decorator of secondDecorators) {
        let decoratorSearcher = RegExp('(implements .*' + decorator + '|(@|ng)' + decorator + '[^a-zA-Z])');
        editor.scan(decoratorSearcher, (result) => {
          let isImportedSearch = new RegExp(decorator + '.* ?} from \'@angular\/core');
          let abort = false;
          editor.scan(isImportedSearch, (result) => {
      ...
      editor.scan(/ ?} from '@angular\/core'/, (result) => {
        editor.setCursorBufferPosition([result.range.start.row, result.range.start.column], { autoscroll: false });
  ...
  // TODO: change method and properties names
  importClass(basePath = null) {
    ...
    if (editor) {
      ...
      let exportClassName = 'export (static )?((function)|(class)|(interface)) ' + className;
      let exportDefaultClassName = 'export default (static )?((function)|(class)|(interface)) ' + className;
      ...
      for (let exportType of searchExports) {
        ...
        atom.workspace.scan(exportType, (result) => {
          ...
          if (path !== '') {
            ...
            let cursorPosition = editor.getCursorBufferPosition();
            let source_file = path.split('/').pop();
            editor.scan(RegExp(' ?} from (\'|\").*?' + source_file + '(\'|\")'), (result) => {
                // TODO: Add the posiibility to enable and disable autoscroll
                is_imported = true;
                editor.setCursorBufferPosition([ result.range.start.row, result.range.start.column ], { autoscroll: false });
            ...
            if (!is_imported) {
                ...
                if (exportType.source === exportClassName) {
                    editor.insertText('import { ' + className + ' } from \'' + path.replace(/\\/g,"/") + '\';\n');
                } else {
                    editor.insertText('import ' + className + ' from \'' + path.replace(/\\/g,"/") + '\';\n');
                }
                // TODO: remove this line when { autoscroll: false } option works OK
